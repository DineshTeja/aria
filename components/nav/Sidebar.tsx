...
import { Hospital, CommandIcon, XIcon } from "lucide-react"
      ...
      >
        ...
        >
            <div className={cn("flex items-center cursor-pointer", !isCollapsed ? "ml-3" : "")} onClick={() => router.push("/")}>
                <Hospital className="w-7 h-7 text-green-700 font-light" />
                {!isCollapsed && <h2 className="ml-2 mb-1 text-3xl font-light text-green-700">aria</h2>}
        </div>
        </div>
...
const NavButton = ({
  ...
  const content = (
        ...
        >
          ...
          {!isCollapsed && (
                <span className={`text-md ${topLevel ? "font-medium text-green-700" : "font-medium text-green-800"} `}>{item.label}</span>
            )}
